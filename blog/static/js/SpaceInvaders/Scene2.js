class Scene2 extends Phaser.Scene {
    ...
    create() {
        ...
        this.score = 0;
        this.scoreText = this.add.text(860, 10, "Score: " + this.score, textStyle);
        ///// Lives /////
        this.lives = 3;
        this.livesText = this.add.text(315, 10, "Lives: " + this.lives, textStyle);

        ///// GameOver/Correct/Incorrect Text /////
        this.gameOverText = this.add.text(250, 300, "", this.textStyle);
        ...
        var defs = document.getElementById('mydefs').value;
        var terms = document.getElementById('terms').value;

        var words = terms.split(",");
        var definitions = defs.split(",");
        //var words = ["A", "B", "C", "D", "E"];
        //var definitions = ["A_def", "B_def", "C_def", "D_def", "E_def"];

        this.currentIndex = Phaser.Math.Between(0, 3); //current word we are on

        this.wordText = this.add.text(710, 10, "Word: " + words[this.currentIndex], textStyle);

        // enemies defeated
        this.enem_defeated = 0;
        this.defeatedText = this.add.text(860, 40, "Enemies Defeated: " + this.enem_defeated, textStyle);

        // enemy / options display on right

        var displayedOptions = ["test", "test", "test", "test"];

        this.display1 = this.add.text(800, 90, displayedOptions[0], textStyle);
        this.display2 = this.add.text(800, 150, displayedOptions[1], textStyle);
        this.display3 = this.add.text(800, 210, displayedOptions[2], textStyle);
        this.display4 = this.add.text(800, 270, displayedOptions[3], textStyle);
        this.answerIndex = this.updateWords(displayedOptions, definitions, this.currentIndex);

        this.enemy1_display = this.physics.add.sprite(730, 110, "enemies1").setScale(.6);
        this.enemy2_display = this.physics.add.sprite(730, 170, "enemies2").setScale(.6);
        this.enemy3_display = this.physics.add.sprite(730, 230, "enemies3").setScale(.6);
        this.enemy4_display = this.physics.add.sprite(730, 290, "enemies4").setScale(.6);
    ...
    wordClear1(words, definitions, displayedOptions) {
        if (this.answerIndex === 0 && this.enemies1.getChildren().length == 1) {
            this.gameOverText.text = "\t\tGreat Work! \nClick to continue!";
            this.gameOverText.setVisible(true);
            this.gameOverText.setDepth(1);
    ...
    updateWords(displayedOptions, definitions, index) {
        ...
        var answerUsedIndexes = [index];

        do {
            answerIndex = Phaser.Math.Between(0, 3);
        }
        while (answerUsedIndexes.includes(answerIndex));

        answerUsedIndexes.push(answerIndex);
        ...
        for (i = 0; i < 4; i++) {
            if (i != answerIndex) {
                var wrongIndex;
                do {
                    wrongIndex = Phaser.Math.Between(0, 3);
